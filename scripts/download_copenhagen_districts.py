    ...
    for headers in headers_options:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Attempt with headers {headers} failed: {response.status_code}")
...
def download_from_datastore_search() -> dict:
    ...
    url = f"https://admin.opendata.dk/api/3/action/datastore_search"
    params = {
...
def process_api_response(api_data: dict) -> gpd.GeoDataFrame:
    ...
    if 'result' in api_data:
        if 'records' in api_data['result']:
            ...
            raise ValueError(f"Unexpected result structure: {api_data['result'].keys()}")
    else:
    ...
    if geom_col is None:
        logger.warning(f"No geometry column found. Available columns: {df.columns.tolist()}")
    ...
    try:
        ...
        if 'geometry' in gdf.columns:
            ...
            if 'bydel_navn' in gdf.columns:
                logger.info(f"Districts: {', '.join(sorted(gdf['bydel_navn'].tolist()))}")
