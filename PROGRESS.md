...
- [x] **UI facelift — Geometric Minimal+** — new font stack (Outfit headings / Work Sans body / Space Mono labels); pure-white background (`--bg: #ffffff`); neutral-grey border system (`--border`, `--border-subtle`) replacing pervasive blue tint (`--blue-1`) throughout chrome; neutral-grey shadows (no blue cast); step cards: top-rule accent (black on active, grey on inactive) replaces left border; enlarged headings (1.65rem Outfit, −0.025em tracking); ghost step-counter ordinals via CSS counter; monospace `— TAG —` decorative labels; all UI text/buttons/active states use near-black `#111827` — colour reserved for data-viz (map score ramp, demographic bars, legend gradient) and intentional accents only; dev server fixed to use `dist/` build mirroring GitHub Pages structure
